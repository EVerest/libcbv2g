...
SCENARIO("Encode and decode DIN session setup message") {
    ...
    GIVEN("Good case - Encode an SessionSetupReq document") {
        ...
        memset(header.SessionID.bytes, 0x00, din_sessionIDType_BYTES_SIZE);
        ...
        uint8_t evccid[] = {0x84, 0x87, 0x72, 0x60, 0xF3, 0x58, 0x00, 0x00};
        memcpy(body.SessionSetupReq.EVCCID.bytes, evccid, sizeof(evccid));
